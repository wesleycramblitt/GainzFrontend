...
class WorkoutGenerator extends React.Component {
    ...
    downloadRoutine() {
      ...
      doc.text("Custom Workout Routine",pageCenter,40,'center');
      
      doc.setFontStyle("normal");
      doc.setFontSize(16);
      doc.text("Generated By: http://www.gainzgenerator.com",pageCenter,55,'center');
      doc.text("Name: "+this.state.routineData.name,pageCenter,70,'center');
      doc.text("Volume: "+this.state.routineData.volume + " rep(s)",pageCenter,85,'center');
      doc.text("Frequency: "+this.state.routineData.frequency+ " time(s) per week",pageCenter,100,'center');
      
      var repScheme = this.state.routineData.repScheme.sets+" set(s) of "+
      this.state.routineData.repScheme.reps+" rep(s)";
      doc.text("Rep Scheme: "+repScheme,pageCenter,115,'center');
      doc.text("Split: "+this.state.routineData.splitName,pageCenter,130,'center');

      var equipment =  this.state.routineData.exerciseTypes.join(", ");

      doc.text("Equipment: "+equipment,pageCenter,145,'center');
      ...
      var Y = 20;
      for (var i in this.state.routineData.workoutDays) {
          doc.setFontStyle("bold");
          var workoutDay = this.state.routineData.workoutDays[i];
          if (workoutDay.isRest) {
            doc.text("Day "+(Number(i)+1)+" - Rest" , 15,Y);
          }
          else
          {
            doc.text("Day "+(Number(i)+1).toString()+" - "+workoutDay.name , 15,Y);
          }
          ...
          for (var j in workoutDay.workouts) {
              ...
              var repScheme
              doc.text(workout.exercise.name+ " - " + repScheme, 20,Y);
              doc.setTextColor(0,200,0);
              var demo = "https://www.google.com/search?q="+workout.exercise.name.replace(" ", "+")
              doc.textWithLink("Demonstration",150, Y,{url:demo})
              const textWidth = doc.getTextWidth("Demonstration");
              doc.line(150, Y, 150 + textWidth, Y);
